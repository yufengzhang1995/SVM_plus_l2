function model = LULUPAPI(labels, K, tK, C, C_star,gamma,z,m)
    n       = length(labels);
    uy      = unique(labels);
    ...
    Z_p = repmat(z(1:m)',m,1); 
    Z_e = z(m+1:n); 
    
    tK_star = Z_p .* tK ;    % multipled by label uncertainty
    
    % for privileged information kernel
    H_p = tK * inv(gamma * eye(m) + C_star * tK_star);
